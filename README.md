...
在 [Stash](https://stash.ws) 面板上一键检测 AI 服务与流媒体的可用性及出口地区。提供 **合并包**（`global.stoverride`）或 **AI 包** + **流媒体包** 分拆安装，三选一即可，勿重复安装合并包与两个分包以免磁贴重复。

图标与脚本优先通过 [jsDelivr（fastly 前缀）](https://fastly.jsdelivr.net/) 拉取，便于缓存与访问。

## 支持的服务

**合并包**（`global.stoverride`）= 下表 AI + 流媒体全部磁贴。
...
打开 Stash → 覆写 → 安装覆写，填入对应 URL。若此前安装过 `global-ai-availability.stoverride` / `global-streaming.stoverride`，请删除旧覆写后改用下方新路径。
...
基于上游 [StashNetworks/misc](https://github.com/StashNetworks/misc) 开发；本仓库扩展了 Codex、Claude、Grok、Gemini、Copilot、Disney+、Spotify 等检测，提供合并包 `global.stoverride` 与分拆包 `ai-availability.stoverride` / `streaming.stoverride`，合并 ChatGPT Web/App 为单一磁贴，并改进了 YouTube、Spotify 等脚本的检测逻辑。
